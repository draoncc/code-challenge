...
function dind::init {
  ...
  local local_host=$(docker-machine ip ${DOCKER_MACHINE_NAME})
  if [[ ${IP_MODE} = "ipv6" ]]; then
      local local_host=$(docker-machine ip ${DOCKER_MACHINE_NAME})
  fi
...
function dind::create-node-container {
  ...
  # MODIFIED
  until [ "$(docker inspect -f {{.State.Running}} kube-node-${next_node_index})" == "true" ]; do
    sleep 0.1;
  done;
  # While updaing the trust chain on each node would have
  # been preferrable over setting the registry as insecure,
  # I did not manage to get it to work. The script would
  # halt and the containers would throw errors and not run.
  # 
  # docker exec kube-node-${next_node_index} update-ca-certificates
  #
  # Copy insecure registry daemon config instead
  #
  docker exec kube-node-${next_node_index} mkdir -p /etc/docker
  docker cp \
         k8s-daemon.json \
         kube-node-${next_node_index}:/etc/docker/daemon.json
  docker exec kube-node-${next_node_index} systemctl restart docker
}
...
function dind::wait-for-ready {
  ...
  local_host=$(docker-machine ip ${DOCKER_MACHINE_NAME})
  if [[ ${IP_MODE} = "ipv6" ]]; then
      local_host=$(docker-machine ip ${DOCKER_MACHINE_NAME})
  fi
